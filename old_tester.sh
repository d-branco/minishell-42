...
echo -n "Running test: Run invalid command                            "
$MINISHELL_PATH <<< "invalid\"command" >> output.txt 2>&1
MINISHELL_EXIT_CODE=$?
if [ $MINISHELL_EXIT_CODE -eq 2 ] && grep -q "command not found" output.txt; then
    echo "    GOOD!"
else
...
echo "" >> output.txt
# Test 7 Check if invalid command returns an error
echo -n "Running test: Run invalid command                            "
$MINISHELL_PATH <<< "invalid\"command" >> output.txt 2>&1
MINISHELL_EXIT_CODE=$?
if [ $MINISHELL_EXIT_CODE -ne 0 ] && grep -q "command not found" output.txt; then
    echo "    GOOD!"
else
    echo -e "\n    Test Failed: Invalid command did not produce an error."
fi
